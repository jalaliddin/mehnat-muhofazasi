<template>
  <div>
    <div class="d-flex align-center justify-space-between mb-6">
      <div class="text-h5 font-weight-bold" style="color: #0D1B2A">Bo'limlar</div>
    ...
    <v-dialog v-model="dialog" max-width="500">
      <v-card rounded="xl">
        <v-card-title class="pa-6 d-flex align-center">
          ...
          {{ editItem ? "Bo'limni tahrirlash" : "Yangi Bo'lim" }}
        </v-card-title>
        <v-divider />
    ...
    <ConfirmDialog
      ...
      :message="`'${deleteItem?.name}' bo'limini o'chirmoqchimisiz?`"
      @confirm="deleteDept"
    />
  </div>
</template>
...
const filteredDepts = computed(() => {
  ...
  if (filterOrg.value) list = list.filter(d => d.organization_id === filterOrg.value)
  if (search.value) {
...
async function save() {
  ...
  try {
    if (editItem.value) {
      await api.put(`/departments/${editItem.value.id}`, form.value)
    } else {
...
async function deleteDept() {
  try {
    await api.delete(`/departments/${deleteItem.value.id}`)
    await fetchAll()
  } catch (e) {}
}
